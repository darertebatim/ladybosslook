...
function SortableRearrangeItem({ task }: { task: RoutineTask }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-xs text-muted-foreground tabular-nums">{task.targetSeconds > 0 ? `${(task as any).isEstimate ? '~' : ''}${Math.round(task.targetSeconds / 60)}m` : '⏱️'}</span>
        ...
        <div className="flex-1 overflow-y-auto space-y-2 mb-4">
          {currentTask && (
            <div className="flex items-center gap-3 bg-foreground/[0.04] rounded-2xl px-4 py-3.5 opacity-40">
              ...
              <span className="text-xs text-muted-foreground tabular-nums">{currentTask.targetSeconds > 0 ? `${(currentTask as any).isEstimate ? '~' : ''}${Math.round(currentTask.targetSeconds / 60)}m` : '⏱️'}</span>
            </div>
          )}
    ...
    <div className="fixed inset-0 z-[9999] flex flex-col bg-[#f5f3ef]">
      ...
      {showRearrangeSheet && (
        <RearrangeSheet
          ...
          setRearrangeTasks={setRearrangeTasks}
          onDone={() => {
            haptic.medium();
          ...
          onClose={() => setShowRearrangeSheet(false)}
        />
      )}
