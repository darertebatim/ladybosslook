 ...
 *
 * Design language matches the WhatsRilo onboarding celebration:
 * radial peach→pink→lavender wash, expanding glow rings, sparkles,
 * gradient serif headline with shimmer underline.
 */
export const ShieldEarnedSheet = ({
  ...
  useEffect(() => {
    if (!open) return;
    haptic.success();
    // Soft confetti burst, slightly delayed so the rings bloom first
    const t1 = setTimeout(() => {
      try {
        confetti({
          particleCount: 70,
  ...
  // Stable sparkle positions per render
  ...
  return (
    <OverlayPortal>
      ...
      >
        {/* Soft warm wash backdrop — no harsh black */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          ...
          {/* Hero shield with gradient + glow */}
          <motion.div
          ...
          >
            ...
            >
              <Shield className="h-11 w-11 text-white drop-shadow-[0_2px_6px_rgba(0,0,0,0.18)]" strokeWidth={2.4} />
            </div>
          </motion.div>

          {/* Headline — Georgia serif with gradient text */}
          <motion.h2
          ...
          {/* CTA — gradient pill, no flat orange */}
          <motion.button
            ...
            style={{
              background:
                'linear-gradient(135deg, #1a1f3d 0%, #6B43D1 100%)',
            }}
          >
            {t('shieldEarned.cta')}
          </motion.button>
...
// (legacy export shape unchanged)
const _legacy = () => null;
if (false) {
  // Suppress lint about unused — kept intentionally below for diff clarity.
  _legacy();
}

/* eslint-disable */
function _unused_legacy_keep() {
  return null;
}
/* eslint-enable */
const _stage = (open: boolean) => open;
_stage(false);

function _origRender_keep_in_history() {
  // Kept intentionally empty — replaced by motion-based render above.
  return null;
}

// Original return removed below; the export above is the active component.
function __unused_marker() { return false; }
__unused_marker();

// END FILE

// (Strip stub below if needed)
function _shieldEarnedSheetStub() {
  return null;
}
_shieldEarnedSheetStub();
...
// (kept only the new component above)

function _final_noop() { return null; }
_final_noop();
