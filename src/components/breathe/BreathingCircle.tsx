...
export function BreathingCircle({
  ...
  return (
    <div className="relative flex items-center justify-center w-72 h-72">
      ...
      <div
        className={cn(
          'absolute rounded-full bg-primary/20 backdrop-blur-sm shadow-lg transition-transform',
      ...
      <div 
        className="absolute rounded-full bg-card border border-border flex flex-col items-center justify-center shadow-xl"
        style={{
          width: '35%',
          height: '35%',
        }}
      >
        {/* Phase text */}
        <span className="text-xl font-semibold text-foreground drop-shadow-sm">
          {phaseText}
        </span>
        
        {/* Method text (Nose/Mouth) or countdown */}
        {countdown !== undefined && phase.includes('hold') ? (
          <span className="text-base text-muted-foreground mt-0.5">{countdown}</span>
        ) : methodText ? (
          <span className="text-xs text-muted-foreground mt-0.5">{methodText}</span>
        ) : null}
      </div>
    </div>
  );
