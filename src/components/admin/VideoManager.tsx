...
import { Loader2, Trash2, Upload, Pencil, Link, Video } from "lucide-react";
...
export const VideoManager = () => {
  ...
  const [formData, setFormData] = useState({
    title: "", description: "", playlist_id: "", url: "", is_vertical: false,
  });
  ...
  const uploadMutation = useMutation({
    mutationFn: async () => {
      if (urlMode) {
        // URL mode - paste YouTube/Vimeo URL
    ...
    onSuccess: () => {
      ...
      setFormData({ title: '', description: '', playlist_id: '', url: '', is_vertical: false });
      setVideoFiles([]);
    ...
    <div className="space-y-6">
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <form onSubmit={(e) => { e.preventDefault(); if (!editingVideo) return; updateMutation.mutate({ id: editingVideo.id, updates: { title: formData.title, description: formData.description, is_vertical: formData.is_vertical }, playlistId: formData.playlist_id }); }} className="space-y-4">
