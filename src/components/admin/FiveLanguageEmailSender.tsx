...
export function FiveLanguageEmailSender() {
  ...
  const [addMissingResult, setAddMissingResult] = useState<AddMissingResult | null>(null);
  ...
  const addMissingBuyers = async () => {
    setAddingMissing(true);
    setAddMissingResult(null);
    ...
    try {
      ...
      const response = await supabase.functions.invoke("add-missing-five-language-buyers", {
        headers: {
          Authorization: `Bearer ${session.access_token}`,
        },
      });
      ...
      setAddMissingResult(response.data);
      
      if (response.data.added > 0) {
        toast.success(`Successfully added ${response.data.added} new members!`);
      }
      if (response.data.alreadyExists > 0) {
        toast.info(`${response.data.alreadyExists} members already existed and were tagged`);
      }
      if (response.data.failed > 0) {
        toast.warning(`${response.data.failed} failed`);
      }
    } catch (error: any) {
      console.error("Error adding missing buyers:", error);
      toast.error(`Failed to add missing buyers: ${error.message}`);
    } finally {
  ...
  return (
    <div className="space-y-6">
      {/* Add Missing Customers Card */}
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Tag className="h-5 w-5 text-orange-500" />
            Add Missing Five Language Customers
          </CardTitle>
          <CardDescription>
            Add the 14 customers from first day payment bug to Mailchimp with tags
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button 
            onClick={addMissingBuyers} 
          ...
          >
            {addingMissing ? (
              <>
                ...
                Adding 14 Missing Customers...
              </>
            ) : (
              <>
                <Tag className="mr-2 h-4 w-4" />
                Add 14 Missing Customers to Mailchimp
              </>
            )}
          </Button>
          ...
          {addMissingResult && (
            <div className="space-y-3">
              <div className={`flex items-center gap-2 p-3 rounded-lg ${
                addMissingResult.failed === 0 ? "bg-green-500/10 text-green-600" : "bg-yellow-500/10 text-yellow-600"
              }`}>
                {addMissingResult.failed === 0 ? (
                  <CheckCircle className="h-5 w-5" />
                ) : (
                  <AlertCircle className="h-5 w-5" />
                )}
                <span className="font-medium">{addMissingResult.message}</span>
              </div>

              {addMissingResult.emails.length > 0 && (
                <div className="text-sm">
                  <p className="font-medium text-green-600 mb-1">✓ Added/Tagged:</p>
                  <div className="max-h-40 overflow-y-auto bg-muted/30 rounded p-2">
                    {addMissingResult.emails.map((email, i) => (
