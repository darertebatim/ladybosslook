...
export function ActiveRoundsCarousel({ 
  ...
  return (
    <div className={`tour-programs-carousel bg-white rounded-lg border border-border shadow-sm ${isCollapsed ? 'p-2' : 'space-y-2 p-2'}`}>
      ...
      <button 
        className={`w-full flex items-center justify-between px-2 py-1 rounded-sm transition-all active:bg-black/5 ${
          isCollapsed ? 'border border-border/50' : ''
        }`}
