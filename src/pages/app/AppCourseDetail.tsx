...
const AppCourseDetail = () => {
  ...
  const [showCourseNotificationPrompt, setShowCourseNotificationPrompt] =
    useState(false);
  const [showSessionReminderSheet, setShowSessionReminderSheet] =
    useState(false);
  ...
  const handleToggleWaitlist = async () => {
    ...
    try {
      if (isOnWaitlist) {
        ...
        await supabase
          .from("program_waitlist")
          .insert({ user_id: user.id, program_slug: slug });
        ...
        toast.success(
          "You're on the waitlist! A message was sent to support.",
          {
            action: {
              label: "View Chat",
  ...
  useEffect(() => {
    const checkCalendarPrompt = async () => {
      ...
      const dismissed = localStorage.getItem(
        "calendarPermissionPromptDismissed",
      );
      if (dismissed) {
        const daysSince =
  ...
  const generateAllSessionEvents = (): CalendarEvent[] => {
    ...
    if (dbSessions && dbSessions.length > 0) {
      return dbSessions.map((session) => {
        ...
        return {
          ...
          description: getEventDescription(
            session.description ||
              `Session ${session.session_number} of ${program.title}`,
          ),
          startDate: new Date(session.session_date),
          endDate: new Date(
            new Date(session.session_date).getTime() +
              (session.duration_minutes || 90) * 60000,
          ),
    ...
    while (currentDate <= endDate) {
      events.push({
        ...
        description: getEventDescription(
          `Session ${sessionNumber} of ${program.title}`,
        ),
  ...
  const handleSyncAllSessions = async () => {
    ...
    if (isNativeApp() && isCalendarAvailable()) {
      ...
      try {
        ...
        const result = await addMultipleEventsToCalendar(
          events,
          oldCalEventIds,
        );
        ...
        if (result.success) {
          toast.success(
            `Added ${result.addedCount} sessions to your calendar!`,
          );
  ...
  const handleAddSingleSession = async (
    ...
    const event: CalendarEvent = {
      ...
      endDate: new Date(
        sessionDate.getTime() + (session.duration_minutes || 90) * 60000,
      ),
    ...
    if (settings.enabled && isLocalNotificationsAvailable()) {
      ...
      for (const item of items) {
        ...
        if (unlockDate && unlockDate > new Date()) {
          const title =
            (item as any).title ||
            (item as any).audio_content?.title ||
            "Content Unlocked";
          if (settings.isUrgent) {
            await scheduleUrgentAlarm({
  ...
  const handleContentReminder = async (item: any, unlockDate: Date) => {
    ...
    if (hasReminder) {
      ...
      const title =
        item.title || item.audio_content?.title || "Content Unlocked";
  ...
  const getNextUpcomingSession = () => {
    ...
    const now = new Date();
    return (
      dbSessions.find((session) => new Date(session.session_date) > now) || null
    );
  };
  ...
  const hasDripModules =
    playlistModules && playlistModules.some((m) => m.drip_delay_days > 0);

  // Fallback: Check if there are drip-scheduled tracks (e.g., audiobook chapters)
  const hasDripTracks =
    !hasDripModules &&
    playlistTracks &&
    playlistTracks.some((t) => t.drip_delay_days > 0);
  ...
  const enrollMutation = useMutation({
    ...
    onMutate: async () => {
      ...
      const previousEnrollments = queryClient.getQueryData<string[]>([
        "enrollments",
        user?.id,
      ]);
  ...
  return (
    <>
      <div className="flex flex-col h-full overflow-hidden bg-background">
        ...
        <div
          className="fixed top-0 left-0 right-0 z-50 bg-card-warm/85 backdrop-blur-lg rounded-b-3xl shadow-ios"
        ...
        <div
          className="shrink-0"
          style={{ height: "calc(76px + env(safe-area-inset-top, 0px))" }}
        />
                                ...
                                {!isPast && (
                                  <Button
                                    ...
                                    className={cn(
                                      ...
                                      index ===
                                        dbSessions.findIndex(
                                          (s) => !isSessionPast(s.session_date),
                                        ) && "tour-session-calendar-icon",
                                    )}
                                    onClick={() =>
                                      handleAddSingleSession(session)
                                    }
                    ...
                    <Card className="tour-content-schedule">
                      ...
                      <CardContent>
                        ...
                        <div className="space-y-3">
                          ...
                          {hasDripModules &&
                            playlistModules?.map((module, moduleIndex) => {
                              const unlockDate = getModuleUnlockDate(
                                module.drip_delay_days,
                              );
                              const isAvailable = unlockDate
                                ? new Date() >= unlockDate
                                : true;
                              const isToday =
                                  ...
                                  {!isAvailable && unlockDate && (
                                    <Button
                                      ...
                                      className={cn(
                                        ...
                                        moduleIndex ===
                                          playlistModules?.findIndex((m) => {
                                            const ud = getModuleUnlockDate(
                                              m.drip_delay_days,
                                            );
      ...
      <AlertDialog
        open={showEnrollmentReminder}
        onOpenChange={setShowEnrollmentReminder}
      >
        <AlertDialogContent className="max-w-[90%] sm:max-w-md">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-center text-base">
              You just enrolled! Enable notifications so you never miss class
              reminders
            </AlertDialogDescription>
          </AlertDialogHeader>
