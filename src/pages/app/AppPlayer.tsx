...
export default function AppPlayer() {
  ...
  const { playlists, playlistItems, progressData, enrollments, isLoading } = usePlayerData();
  ...
  const getPlaylistStats = (playlistId: string) =>
    playlistStats.get(playlistId) || { trackCount: 0, totalDuration: 0, completedTracks: 0, coverImage: null };
  ...
  const isPlaylistAvailableOnMobile = (playlist: any) => {
    if (playlist.program_slug && enrollments?.includes(playlist.program_slug)) return true;
  ...
  const availableCategoriesSet = new Set(
    playlists?.filter(p => !p.is_hidden && isPlaylistAvailableOnMobile(p))
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col h-full overflow-hidden bg-background">
        <div className="px-4 pt-4 space-y-3" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 1rem)' }}>
          <Skeleton className="h-8 w-24" />
          <div className="flex gap-2">{[...Array(4)].map((_, i) => <Skeleton key={i} className="h-7 w-16 rounded-full" />)}</div>
          <div className="flex gap-2">{[...Array(3)].map((_, i) => <Skeleton key={i} className="h-7 w-20 rounded-full" />)}</div>
        </div>
