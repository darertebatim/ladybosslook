...
import {
  JournalEntryCard,
  formatDateGroup,
} from "@/components/app/JournalEntryCard";
...
const AppJournal = () => {
  ...
  try {
    routinePlayer = useRoutinePlayerContext();
  } catch {
    /* not available */
  }
  ...
  const stats = useMemo(() => {
    ...
    entries.forEach((entry) => {
      journalDays.add(
        format(startOfDay(new Date(entry.created_at)), "yyyy-MM-dd"),
      );
    });
