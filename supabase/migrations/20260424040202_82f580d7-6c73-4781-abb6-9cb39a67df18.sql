...
BEGIN
  FOR v_user_id IN
    ...
    WHERE p.created_at >= TIMESTAMPTZ '2026-04-24 03:44:34+00'
      AND NOT EXISTS (
        SELECT 1
        FROM public.user_routines_bank urb
        WHERE urb.user_id = p.id
          AND urb.routine_id = '6c2d0492-9310-46a2-99ad-be5c2ddbc3f6'::uuid
      )
  LOOP
    BEGIN
      PERFORM public.provision_daily_reset_for_user(v_user_id);
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'Backfill daily reset failed for %: %', v_user_id, SQLERRM;
    END;
  END LOOP;
END $$;