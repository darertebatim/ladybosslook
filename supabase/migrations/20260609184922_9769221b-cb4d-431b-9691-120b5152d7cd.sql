...
ALTER TABLE public.app_return_events
  ADD COLUMN IF NOT EXISTS platform text;

CREATE INDEX IF NOT EXISTS app_return_events_user_platform_idx
  ON public.app_return_events (user_id, platform);
  ...
  WITH 
    ...
    user_platforms AS (
      ...
      SELECT user_id, platform FROM app_installations
        WHERE user_id IS NOT NULL AND platform IN ('ios','android','web')
      UNION
      SELECT user_id, platform FROM app_return_events
        WHERE user_id IS NOT NULL AND platform IN ('ios','android','web')
    ),
    user_platform_summary AS (
      SELECT user_id,
        ...
        bool_or(platform='web') AS has_web
      ...
      SELECT k, count(*)::int AS c FROM (
        SELECT p.id,
          CASE
            ...
            WHEN ups.has_web THEN 'Web'
            ELSE '— unknown (no signal yet) —'
          END AS k
        FROM profiles p
