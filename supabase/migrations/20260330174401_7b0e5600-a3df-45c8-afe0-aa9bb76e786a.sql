...
-- Step 1: Add missing columns to free_form_reflections
ALTER TABLE public.free_form_reflections 
  ADD COLUMN IF NOT EXISTS mood text,
  ADD COLUMN IF NOT EXISTS shared_with_admin boolean DEFAULT false,
  ADD COLUMN IF NOT EXISTS shared_at timestamptz,
  ADD COLUMN IF NOT EXISTS updated_at timestamptz DEFAULT now();

-- Step 2: Copy all journal entries into free_form_reflections (skip duplicates by id)
INSERT INTO public.free_form_reflections (id, user_id, title, content, mood, shared_with_admin, shared_at, created_at, updated_at)
SELECT 
  id,
  user_id,
  COALESCE(title, 'Reflection'),
  content,
  mood,
  COALESCE(shared_with_admin, false),
  shared_at,
  created_at,
  updated_at
...
-- Step 3: Update get_home_data function to use free_form_reflections
...
BEGIN
  ...
  -- Changed: use free_form_reflections instead of journal_entries
