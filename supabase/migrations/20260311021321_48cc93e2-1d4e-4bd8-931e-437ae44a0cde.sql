...
CREATE OR REPLACE FUNCTION public.get_program_events_for_date(
  p_user_id uuid,
  p_date_str text
)
RETURNS jsonb
...
BEGIN
  ...
  -- Use the date as-is (caller sends local date), build UTC range for session matching
  v_day_start := v_date::timestamptz;
  v_day_end := (v_date + interval '1 day')::timestamptz;
  ...
  completions AS (
    ...
    WHERE user_id = p_user_id AND completed_date = p_date_str
  ),
  -- Get program titles
  catalog AS (
    SELECT slug, title FROM program_catalog
  ),
  -- Get sessions for all rounds on this date
  round_sessions AS (
    SELECT 
      ps.id,
      ps.round_id,
  ...
  -- Get modules (playlist_supplements) with unlock dates
  module_events AS (
    SELECT jsonb_build_object(
      'id', ps.id,
      'type', 'module',
      ...
      'dripAnchorDate', CASE WHEN ae.is_self_paced THEN ae.enrolled_at ELSE ae.first_session_date::text END,
    ...
    WHERE ae.audio_playlist_id IS NOT NULL
      ...
      AND (CASE WHEN ae.is_self_paced THEN ae.enrolled_at ELSE ae.first_session_date::text END) IS NOT NULL
  ),
  -- Get audio tracks with unlock dates
  track_events AS (
    SELECT jsonb_build_object(
      'id', ac.id,
      'type', 'track',
