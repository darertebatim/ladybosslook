...
BEGIN
  ...
  -- Use the user's profile timezone (fallback to America/Los_Angeles) so that
  -- "today" matches the user's local day, not UTC. This mirrors the one-time
  -- task behavior for instructor-bundle provisioning.
  ...
  playlist_update_events AS (
    ...
    WHERE (api.created_at AT TIME ZONE v_tz)::date = v_date
      AND api.created_at > ap.access_started_at
