...
-- 1. Audio listen events table (per-play row, updated with seconds as user listens)
...
GRANT SELECT, INSERT, UPDATE ON public.audio_listen_events TO authenticated;
GRANT ALL ON public.audio_listen_events TO service_role;

ALTER TABLE public.audio_listen_events ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users insert own audio events"
  ON public.audio_listen_events FOR INSERT TO authenticated
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users update own audio events"
  ON public.audio_listen_events FOR UPDATE TO authenticated
...
CREATE INDEX IF NOT EXISTS audio_listen_events_user_idx
  ON public.audio_listen_events(user_id);
CREATE INDEX IF NOT EXISTS audio_listen_events_audio_idx
  ON public.audio_listen_events(audio_id);
CREATE INDEX IF NOT EXISTS audio_listen_events_created_idx
  ON public.audio_listen_events(created_at);

-- 2. Admin analytics aggregation RPC (bulletproof, server-side)
...
GRANT EXECUTE ON FUNCTION public.get_admin_user_breakdown() TO authenticated;
