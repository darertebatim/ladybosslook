...
-- Table to store admin-triggered round update notifications
CREATE TABLE public.round_update_notifications (
...
-- Table to track which users have read/dismissed the notification
CREATE TABLE public.round_notification_reads (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  notification_id uuid NOT NULL REFERENCES public.round_update_notifications(id) ON DELETE CASCADE,
...
-- Enable RLS
ALTER TABLE public.round_update_notifications ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.round_notification_reads ENABLE ROW LEVEL SECURITY;

-- Notifications: admins can insert, authenticated users can read
CREATE POLICY "Admins can insert notifications"
  ON public.round_update_notifications FOR INSERT TO authenticated
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Authenticated users can read notifications"
  ON public.round_update_notifications FOR SELECT TO authenticated
  USING (true);

-- Reads: users can insert their own reads, and read their own
CREATE POLICY "Users can insert own reads"
  ON public.round_notification_reads FOR INSERT TO authenticated
  WITH CHECK (user_id = auth.uid());

CREATE POLICY "Users can read own reads"
  ON public.round_notification_reads FOR SELECT TO authenticated
  USING (user_id = auth.uid());

-- Update the RPC to include round_update notifications
...
BEGIN
  ...
  module_events AS (
    SELECT jsonb_build_object(
      ...
      'moduleId', ps.id,
      'playlistId', ae.audio_playlist_id,
