...
const CAMPAIGNS: Record<string, Campaign> = {
  ...
  igads: {
    ...
    nextSessionBody: [
      p("جای شما در جلسه زنده وبینار اینستاگرام ادز خالی بود."),
      p("<strong>چیزی که از دست دادید فقط یک ویدیو نبود.</strong> در آن یک ساعت، قدم‌به‌قدم روی صفحه نشان دادم:"),
      `<ul style="margin:0 0 14px;padding:0 20px 0 0;font-size:15px;line-height:2;">
        <li>چرا تبلیغ‌های اینستاگرام کلیک می‌گیرند اما مشتری نمی‌آورند — و دقیقاً کجا بودجه هدر می‌رود</li>
        <li>ساختار کمپینی که برای بیزینس‌های ایرانی در آمریکا و کانادا جواب می‌دهد</li>
        <li>با چه بودجه‌ای شروع کنید که نه کم باشد نه سوخته</li>
        <li>چطور در چند روز بفهمید تبلیغ‌تان دارد جواب می‌دهد یا باید خاموشش کنید</li>
        <li>و اشتباهی که ۹ نفر از ۱۰ نفر در همان قدم اول مرتکب می‌شوند</li>
      </ul>`,
      p("شرکت‌کننده‌ها بعد از همان یک ساعت گفتند دیگر می‌دانند پولشان کجا می‌رود و چه چیزی را باید درست کنند."),
      p("<strong>یک ساعت از وقت‌تان</strong> در مقابل ماه‌ها تبلیغ آزمون‌وخطا و بودجه‌ای که بی‌صدا می‌سوزد."),
      p("یک جلسه زنده دیگر برگزار می‌کنیم — <strong>و این آخرین جلسه رایگان این دوره است.</strong> ظرفیت جلسه زنده محدود است و بعد از آن، این آموزش دیگر رایگان در دسترس نخواهد بود."),
      p("اگر واقعاً می‌خواهید اینستاگرام برایتان مشتری بیاورد، همین حالا جای خودتان را نگه دارید 👇"),
    ].join(""),
    nextSessionAfterCta: [
      p("در صفحه ثبت‌نام، <strong>ساعت مناسب منطقه زمانی خودتان</strong> را انتخاب می‌کنید و لینک ورود برایتان ایمیل می‌شود."),
      p("جلسه ضبط‌شده پخش نمی‌شود؛ فقط همان یک ساعت زنده است — پس تقویم‌تان را خالی کنید."),
    ].join(""),
  },
};
